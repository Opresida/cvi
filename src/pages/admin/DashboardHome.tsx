    ...
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-neutral-200 p-6 flex items-start gap-4">
          ...
          <div>
            <p className="text-sm text-neutral-500">Registro de Ponto</p>
            <p className="text-2xl font-bold text-accent-600">Ativo</p>
          </div>
        </div>

        {isAdmin && (
          <div className="bg-white rounded-2xl border border-neutral-200 p-6 flex items-start gap-4">
            <div className="bg-accent-600 text-white p-3 rounded-xl">
              <Users size={22} aria-hidden="true" />
            </div>
            <div>
              <p className="text-sm text-neutral-500">Funcionários ativos</p>
              <p className="text-2xl font-bold text-neutral-900">{stats.employees}</p>
            </div>
          </div>
        )}

        <div className="bg-white rounded-2xl border border-neutral-200 p-6 flex items-start gap-4">
          <div className="bg-warm-500 text-white p-3 rounded-xl">
            <CalendarCheck size={22} aria-hidden="true" />
          </div>
          <div>
            <p className="text-sm text-neutral-500">{isAdmin ? "Registros hoje (todos)" : "Seus registros hoje"}</p>
            <p className="text-2xl font-bold text-neutral-900">{stats.todayPunches}</p>
          </div>
        </div>

        <div className="bg-white rounded-2xl border border-neutral-200 p-6 flex items-start gap-4">
          <div className="bg-accent-500 text-white p-3 rounded-xl">
            <Activity size={22} aria-hidden="true" />
          </div>
          <div>
            <p className="text-sm text-neutral-500">Status do Sistema</p>
            <p className="text-2xl font-bold text-accent-600">Online</p>
          </div>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-neutral-200 p-6">
        <h2 className="text-lg font-bold text-neutral-900 mb-4">Informações da Sessão</h2>
        <dl className="grid sm:grid-cols-2 gap-4 text-sm">
          <div>
            <dt className="text-neutral-500">Usuário logado</dt>
            <dd className="font-semibold text-neutral-900">{user.name}</dd>
          </div>
          <div>
